...
func NewWriter(w io.Writer, preset Preset) (*Compressor, os.Error) {
	enc := new(Compressor)
...
func NewWriterCustom(w io.Writer, preset Preset, check Checksum, bufsize int) (*Compressor, os.Error) {
	enc := new(Compressor)
	// The zero lzma_stream is the same thing as LZMA_STREAM_INIT.
	enc.writer = w
...
func (enc *Compressor) Write(in []byte) (n int, er os.Error) {
...
func (enc *Compressor) Flush() os.Error {
...
// underlying reader.
func (enc *Compressor) Close() os.Error {
	if enc != nil {
		er := enc.Flush()
